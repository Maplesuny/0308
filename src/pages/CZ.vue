  ...
  <div style="position: relative" class="fit column">
    <div style="display: flex; width: 1450px">
      <div id="chartDiv" style="height: 100%; width: calc(100% - 450px)">
        ...
        <canvas
          id="canvas"
          style="position: absolute; top: 80px; left: 5px"
        ></canvas>
        <canvas
          id="canvas2"
          style="position: absolute; top: 50px; left: 10px; z-index: -1"
        ></canvas>
        <div class="flex flex-center items-center">
          <!-- q-pb-lg  -->
          <q-btn @click="pre_button" v-bind:disabled="page <= Minpage"
            >上一頁</q-btn
          >
          ...
          <q-btn
            @click="send_page"
            class="q-ml-md"
            v-bind:disable="jump_to > Maxpage"
            >送出</q-btn
          >
          ...
          <span>
            <q-btn round @click="getRandomData">
              <q-avatar size="42px">
                <img
                  src="https://img.icons8.com/color/32/000000/microsoft-excel-2019--v1.png"
                />
              </q-avatar>
            </q-btn>
          </span>
...
export default {
  ...
  setup() {
    ...
    function delete_db_url(page, montage) {
      delete_url.value = `http://${host}:${port}/api/v1/delete?page=${page}&montage=${
        montage_type_convert[montage_type.value]
      }`;
    ...
    async function setoption() {
      ...
      myChart.on("brushSelected", function (params) {
        ...
        if (brushComponent.areas[0] !== undefined) {
          ...
          insert_db_url.value = `http://${host}:${port}/api/v1/insert?page=${
            page.value
          }&starttime=${X_start.value}&y_start_value=${Y_start.value}&endtime=${
            X_end.value
          }&y_end_value=${Y_end.value}&title='${
            send_message.value
          }'&range_start=${x_min_end}&range_end=${x_max_end}&range_start1=${y_max_end}&range_end1=${y_min_end}&montage_type=${
            montage_type_convert[montage_type.value]
          }&ip=${Client_ip.value}`;
    ...
    async function getRandomData() {
      let info_url = `http://${host}:${port}/api/v1/get_info?montage=${
        montage_type_convert[montage_type.value]
      }`;
      ...
      // console.log('dfd', kkdata[0][2]['events'])
      for (var i = 0; i < total_number; i++) {
        ...
        let event_word2 = kkdata[0][i]["events"].substring(
          save_dot_list[0] + 1,
          save_dot_list[1]
        );
        let event_word3 = kkdata[0][i]["events"].substring(
          save_dot_list[1] + 1,
          kkdata[0][i]["events"].length
        );
